 ...
 * @author Nicola Asuni
 */
function stringLeftTrim($str, $replace = '') {
    $re_space = array('p' => '[^\S\xa0]', 'm' => '');
    return preg_replace('/^'.$re_space['p'].'+/'.$re_space['m'], $replace, $str);
}
 ...
 * @author Nicola Asuni
 */
function stringRightTrim($str, $replace = '') {
    $re_space = array('p' => '[^\S\xa0]', 'm' => '');
    return preg_replace('/'.$re_space['p'].'+$/'.$re_space['m'], $replace, $str);
}
 ...
 * @author Nicola Asuni
 */
function stringTrim($str, $replace = '') {
    $str = stringLeftTrim($str, $replace);
    $str = stringRightTrim($str, $replace);
    return $str;
}
...
function getHtmlDomArray($html) {
    ...
    while (($offset < strlen($html)) AND ( $pos = strpos($html, '</pre>', $offset)) !== false) {
        ...
        while (preg_match("'<xre([^\>]*)>(.*?)" . $re_space['p'] . "(.*?)</pre>'" . $re_space['m'], $html_b)) {
            // preserve spaces on <pre> tag
            $html_b = preg_replace("'<xre([^\>]*)>(.*?)" . $re_space['p'] . "(.*?)</pre>'" . $re_space['m'], "<xre\\1>\\2&nbsp;\\3</pre>", $html_b);
        }
    ...
    $html = preg_replace('/' . $re_space['p'] . '+<\/(td|th|li|dt|dd)>/' . $re_space['m'], '</\\1>', $html);
    $html = preg_replace('/[\s]+<(tr|td|th|li|dt|dd)/', '<\\1', $html);
    $html = preg_replace('/' . $re_space['p'] . '+<(ul|ol|dl|br)/' . $re_space['m'], '<\\1', $html);
    ...
    $html = preg_replace('/' . $re_space['p'] . '+<img/' . $re_space['m'], chr(32) . '<img', $html);
    ...
    $html = preg_replace('/<li([^\>]*)>' . $re_space['p'] . '*<img/' . $re_space['m'], '<li\\1><font size="1">&nbsp;</font><img', $html);
    ...
    $html = preg_replace('/' . $re_space['p'] . '+/' . $re_space['m'], chr(32), $html); // replace multiple spaces with a single space
    // trim string
    $html = stringTrim($html);
    // fix br tag after li
    ...
    $dom[$key]['fill'] = ((0 % 2) == 0);
    $dom[$key]['clip'] = (0 > 3);
    $dom[$key]['line-height'] = 1.2;
    $dom[$key]['bgcolor'] = false;
    $dom[$key]['fgcolor'] = array('R' => 0, 'G' => 0, 'B' => 0);
    $dom[$key]['strokecolor'] = array('R' => 0, 'G' => 0, 'B' => 0);
    $dom[$key]['align'] = '';
    $dom[$key]['listtype'] = '';
        ...
        if (preg_match($tagpattern, $element)) {
            ...
            if ($element[0] == '/') {
                ...
                if (($dom[$key]['value'] == 'tr') AND ( $dom[($dom[$key]['parent'])]['thead'] === true)) {
                    if (empty_string($dom[($dom[($dom[$key]['parent'])]['parent'])]['thead'])) {
                ...
                if (isset($dom[$key]['attribute']['style']) AND ! empty($dom[$key]['attribute']['style'])) {
                    ...
                    // font color
                    if (isset($dom[$key]['style']['color']) AND ( !empty_string($dom[$key]['style']['color']))) {
                    ...
                    // background color
                    if (isset($dom[$key]['style']['background-color']) AND ( !empty_string($dom[$key]['style']['background-color']))) {
                ...
                if ((($dom[$key]['value'] == 'ul') OR ( $dom[$key]['value'] == 'ol') OR ( $dom[$key]['value'] == 'dl'))
                        AND ( !isset($dom[$key]['align']) OR empty_string($dom[$key]['align']) OR ( $dom[$key]['align'] != 'J'))) {
                    //if ($rtl) {
                        $dom[$key]['align'] = 'R';
                    //} else {
                    //    $dom[$key]['align'] = 'L';
                ...
                if (isset($dom[$key]['attribute']['color']) AND ( !empty_string($dom[$key]['attribute']['color']))) {
                ...
                if (isset($dom[$key]['attribute']['bgcolor']) AND ( !empty_string($dom[$key]['attribute']['bgcolor']))) {
                    $dom[$key]['bgcolor'] = TCPDF_COLORS::convertHTMLColorToDec($dom[$key]['attribute']['bgcolor'], $this->spot_colors);
                }
                // set stroke color attribute
                if (isset($dom[$key]['attribute']['strokecolor']) AND ( !empty_string($dom[$key]['attribute']['strokecolor']))) {
                ...
                // check for text alignment
                if (isset($dom[$key]['attribute']['align']) AND ( !empty_string($dom[$key]['attribute']['align'])) AND ( $dom[$key]['value'] !== 'img')) {
            ...
            $dom[$key]['value'] = stripslashes(unhtmlentities($element));
        }
        ++$elkey;
        ++$key;
    }
